...
def main(
    paths: Annotated[
        list[Path] | None,
        cyclopts.Parameter(help="Files or directories to check. Defaults to the current directory."),
    ] = None,
    *,
    as_json: Annotated[
        bool,
        cyclopts.Parameter(name=["--json"], help="Emit JSON instead of a human-readable list."),
    ] = False,
) -> int:
    ...
    if as_json:
        json.dump(
            [{"source": str(b.source), "target": b.target, "line": b.line} for b in broken],
            sys.stdout,
            indent=2,
        )
        sys.stdout.write("\n")
